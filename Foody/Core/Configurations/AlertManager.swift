...
struct AlertManager {
    ...
    static func showAlertWithButton(on vc: UIViewController, title: String, message: String?, buttonText: String, completion: @escaping () -> Void) {
            DispatchQueue.main.async {
                let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
    ...
    static func showAlertWithTwoButton(on vc: UIViewController, title: String, message: String?, button1Text: String, button2Text: String, button1Action: @escaping () -> Void, button2Action: @escaping () -> Void) {
        DispatchQueue.main.async {
            let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
            let button1 = UIAlertAction(title: button1Text, style: .default) { _ in
                button1Action()
            }
            alert.addAction(button1)
            let button2 = UIAlertAction(title: button2Text, style: .default) { _ in
                button2Action()
            }
...
extension AlertManager {
    ...
    public static func showInvalidEmailOrPasswordAlert(on vc: UIViewController) {
        self.showBasicAlert(on: vc, title: "Invalid Password or email", message: "Please enter a valid password and email")
    }
